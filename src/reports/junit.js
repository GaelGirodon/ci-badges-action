 ...
 * @param report Unmarshalled JUnit test report
 * @returns {{ tests: number, failed: number, skipped: number }|null}
 */
function parseTestSuitesTag(report) {
  const attr = { tests: 'tests', failed: 'failures', skipped: 'skipped' };
  const ts = report['testsuites'];
  return ts && Object.values(attr).every(attr => ts[`@_${attr}`])
    ? Object.keys(attr).reduce((data, k) => ({ ...data, [k]: parseInt(ts[`@_${attr[k]}`]) }), {})
    : null;
}
